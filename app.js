...
function closeTopLayer() {
  ...
  const modals = [modalTrackMenu, modalRename, modalNewList, modalAddToList, modalTheme, modalSleep];
  ...
  return tracks.map((track, index) => {
    ...
    return `
      <article class="playlist-item ${isActive ? 'active' : ''}" data-id="${track.id}" role="button" tabindex="0">
        ...
        <button class="row-icon-btn row-menu-btn" data-action="menu" data-id="${track.id}" aria-label="Song options"><i class="ti ti-dots-vertical"></i></button>
      </article>
    `;
  }).join('');
}
...
function renderInlineLyrics(idx, wordIdx, lines) {
  ...
  updateLyricsContainer(inlineLyricsFlow, idx, wordIdx, { ratio: 0.68 });
  updateLyricsContainer(fullscreenLyricsFlow, idx, wordIdx, { ratio: 0.58 });
}
...
function openYoutubeSearch() {
  const query = (onlineSearchInput.value || searchInput.value || '').trim();
